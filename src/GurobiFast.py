...
class Solution_Gurobi:
    ...
    def run_algorithm_2(self):
        uniqueList = list(set([item for sublist in self.list_of_selected_solution for item in sublist]))
        of = self.construct_solution_kgpdp_binary_PR_2(k=self.groups, p=self.p, time_max=self.max_time,
                                                       uniqueList1 = self.list_of_selected_solution[0],
                                                       uniqueList2 = self.list_of_selected_solution[1],
                                                       grasp_bound=self.grasp_bound)
        return of
    ...
    def construct_solution_kgpdp_binary_PR_2(self, k, p, time_max, uniqueList1, uniqueList2, grasp_bound):
        #pdp
        instance = self.instance
        n = instance.n
        model = pyo.ConcreteModel()

        uniqueList = list(set(uniqueList1 + uniqueList2))
        sorted_distances = list(dict.fromkeys([i.distance for i in self.sorted_distances if (i.v1 in uniqueList and i.v2 in uniqueList)]))
        bound = self.find_last_more_than_bound(sorted_distances,grasp_bound)
        sorted_distances.reverse()
        Dm = np.max(sorted_distances)
        ...
        x_sum = sum([X[i, ki] for ki in range(k) for i in range(l) if uniqueList[i] in uniqueList2 and uniqueList[i] not in uniqueList1])
        model.C4.add(expr=x_sum >= 1)
        print("c4 Built")
