...
class Solution_Gurobi:
    ...
    def run_algorithm(self):
        ...
        while loop:
            ...
            if (self.max_time - current_time > 1):
                ...
                of = self.construct_solution_kgpdp_packing_gurobipy(k=self.groups, p=self.p, time_max=self.max_time - current_time, bound=bound)
                if of > 0:
                    bound = of
                    best_of = of
    ...
    def construct_solution_kgpdp_packing_gurobipy(self, k, p, time_max, bound):
        # pdp
        instance = self.instance
        n = instance.n
        model = Model("MMDP")
        model.setParam("LogToConsole", 0)
        model.setParam("TimeLimit", time_max)
        ...
        for t in range(k):
            for i in range(n):
                for j in range(i + 1, n):
                    if round(self.distance[i,j], 2) < bound:
        ...
        if status == GRB.OPTIMAL:
            ...
            for v in model.getVars():
                if v.x > 0 and "x" in v.varName:
                    ...
                    sol.append((id, idt))
            dmin = round(self.evalSolution(self.distance, sol), 2)
            return(dmin)
